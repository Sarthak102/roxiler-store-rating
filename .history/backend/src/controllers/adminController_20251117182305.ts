...
// replace the existing listUsers with this fixed version
export async function listUsers(req: Request, res: Response) {
  try {
    const q = String(req.query.q ?? "").trim();
    const role = String(req.query.role ?? "").trim();
    ...
    // main data query (select columns)
    let builder = db("users").select(
      "id",
      "name",
      "email",
    ...
    // build a separate count query with the same filters (avoids GROUP BY issue)
