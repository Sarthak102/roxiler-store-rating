...
// DEV debug replacement for listUsers - paste over the existing listUsers function
export async function listUsers(req: Request, res: Response) {
  try {
    const q = String(req.query.q ?? "").trim();
    const role = String(req.query.role ?? "").trim();
    ...
    if (q) {
      const like = `%${q}%`;
      // portable search avoiding `ilike`
      builder = builder.where(function () {
    ...
    const storeOwnerIds = data.filter((u: any) => u.role === "store_owner").map((u: any) => u.id);
    if (storeOwnerIds.length > 0) {
      const ratings = await db("stores")
        .leftJoin("ratings", "stores.id", "ratings.store_id")
        .select("stores.owner_id as owner_id")
    ...
    // DEV: include stack in response so the frontend / curl shows details
