...
export async function listUsers(req: Request, res: Response) {
  try {
    ...
    // Count query (same filters, no GROUP BY issues)
    let countQuery = db("users").count<{ c: string }>("* as c");
    if (q) {
      const like = `%${q}%`;
    ...
    // Add owner rating for store_owner users
  ...
  try {
    ...
    if (user.role === "store_owner") {
      const row = await db("stores")
        ...
        .avg("ratings.rating as avg_rating")
        .first();
      (user as any).rating = row ? Number(row.avg_rating ?? 0) : null;
    }
